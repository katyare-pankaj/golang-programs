...
func worker(id int, work chan int, results chan<- int, wg *sync.WaitGroup) {
	defer wg.Done()
	for num := range work {
...
func main() {
	...
	workChan := make(chan int, numNumbers)
	resultsChan := make(chan int, numNumbers)

	// Start worker goroutines
	for i := 1; i <= numWorkers; i++ {
		go worker(i, workChan, resultsChan, &wg)
	}
	...
	go func() {
		...
		close(workChan) // Signal to workers that no more work is coming
	}()
	...
	go func() {
		...
		close(resultsChan) // Close the channel after all results are received
	}()
