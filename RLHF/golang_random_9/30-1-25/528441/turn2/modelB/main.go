...
func (lb *LoadBalancer) checkLoad() {
	...
	lb.loadFactor = float32(totalTasks) / float32(len(lb.workers)*bufferSize)
	...
	if lb.loadFactor > maxLoadFactor {
		...
		w.start()
		lb.workers = append(lb.workers, w)
	} else if lb.loadFactor < minLoadFactor && len(lb.workers) > numWorkers {
		// If load factor is below the threshold and there are more than the initial number of workers, stop a worker
		w := lb.workers[len(lb.workers)-1]
		w.stop()
...
func main() {
	...
	go func() {
		for i := 0; i < 100; i++ {
			task := rand.Intn(500) + 1 // Generate random task duration between 1 and 500 ms
			lb.taskChannel <- task
		}
