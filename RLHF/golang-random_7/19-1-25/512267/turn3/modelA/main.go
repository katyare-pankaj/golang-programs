...
func handleRequest(w http.ResponseWriter, r *http.Request) {
	...
	// Get the cache expiration time from the query parameter, default to 2 minutes
	expireStr := values.Get("expire")
	expireDuration, err := time.ParseDuration(expireStr)
	if err != nil {
		expireDuration = time.Duration(2) * time.Minute // Default expiration time
	}
	...
	// Validate cache
	if ok {
		cacheExpiration := time.Now().Add(-expireDuration)
