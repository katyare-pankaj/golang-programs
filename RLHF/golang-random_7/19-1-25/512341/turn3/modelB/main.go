...
// AIModel represents a simplified AI model capable of registering and executing callbacks concurrently.
type AIModel struct {
...
func main() {
	...
	model.AddCallback(func(input, output *string) {
		time.Sleep(500 * time.Millisecond) // Simulate a long-running operation.
		fmt.Println("Long-running callback completed.")
	})
