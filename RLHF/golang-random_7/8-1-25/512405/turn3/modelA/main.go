...
const (
	...
	consumerTimeout = 5 * time.Second // Timeout for consumers
)
...
func consumer(id int, dataCh <-chan int, wg *sync.WaitGroup) {
	...
	for {
		...
		case <-time.After(consumerTimeout):
			// No data received in the specified timeout period, exit the loop
			fmt.Printf("Consumer %d: Timeout reached, exiting\n", id)
			return
		}
	}
}
