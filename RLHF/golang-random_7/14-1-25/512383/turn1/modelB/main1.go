...
func main() {
	...
	// Writer goroutines (write initial values)
	for i := 1; i <= 3; i++ {
		go func(writerID int) {
			for {
				key := writerID * 3 // Select a cell to write based on writer ID
				table.UpdateCell(key, writerID*10)
				fmt.Printf("Writer %d updates cell %d\n", writerID, key)
	...
	// Add a time delay to allow goroutines to output results
