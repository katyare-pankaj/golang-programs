...
func (s *SenderImpl) Send(email *Email) error {
	...
	return s.transport.Send(email) // Call Send here
}

// Make Transport implement Sender interface
// This method allows Transport to satisfy the Sender interface.
func (t *Transport) Send(email *Email) error {
	return t.SendEmail(email)
}
